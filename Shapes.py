...
class Rectangle(Shape):
    def __init__(self, width: float, height: float):
        if width <= 0 or height <= 0:
            raise ValueError(f"Both width and height must be positive and non-zero. Received width: {width} height: {height}")
        self.width = width
        self.height = height
...
class Circle(Shape):
    def __init__(self, radius: float):
        if radius <= 0:
            raise ValueError(f"Radius must be positive and non-zero. Received radius: {radius}")
        self.radius = radius
